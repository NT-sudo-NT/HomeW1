from main import (factorial, is_prime, unique_sorted, group_by_length)
def test_factorial():
...
def test_group_by_length():
    ...
    assert group_by_length(["hi", "hello", "a", "world"]) == {2: ["hi"], 5: ["hello", "world"], 1: ["a"]}

    # Слова одинаковой длины.
    assert group_by_length(["cat", "dog", "bat"]) == {3: ["cat", "dog", "bat"]}

    # Слова разной длины и с повторениями.
    assert group_by_length(["apple", "banana", "pear", "kiwi", "fig", "grape"]) == {3: ["fig"], 4: ["pear", "kiwi"], 5: ["apple", "grape"], 6: ["banana"]}

    # Слова с пробелами.
    assert group_by_length(["hello world", "hi", "python", "code"]) == {11: ["hello world"], 2: ["hi"], 6: ["python"], 4: ["code"]}

    # Слова с цифрами.
    assert group_by_length(["word1", "word2", "123", "4567"]) == {5: ["word1", "word2"], 3: ["123"], 4: ["4567"]}

    # Смешанные символы.
    assert group_by_length(["!@#$%", "abc", "12345", "hello!"]) == {5: ["!@#$%"], 3: ["abc"], 5: ["12345"], 6: ["hello!"]}

    # Слова с разными регистрами.
    assert group_by_length(["Python", "python", "PYTHON"]) == {6: ["Python", "python", "PYTHON"]}

    # Слова с пробелами и смешанными символами.
    assert group_by_length(["Hello World!", "Test 123", "Python3"]) == {12: ["Hello World!"], 9: ["Test 123"], 7: ["Python3"]}
