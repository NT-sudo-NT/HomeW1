...
def group_by_length(words):
    ...
    length_dict = {}  # Инициализируем пустой словарь для хранения результатов
    for word in words:  # Проходим по каждому слову в списке
        length = len(word)  # Вычисляем длину текущего слова
        if length not in length_dict:  # Если длины еще нет в словаре
            length_dict[length] = []  # Создаем новый ключ со списком
        length_dict[length].append(word)  # Добавляем слово в соответствующий список
    return length_dict  # Возвращаем итоговый словарь